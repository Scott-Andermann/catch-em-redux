...
import { addCaught, addSeen, selectSeen, selectCaught } from '../Pokedex/pokedexSlice';
...
const Explore = () => {
    ...
    useEffect(() => {
        ...
        if (pokemonCaught.some(element => element.name === pokemon.name)) setAlreadyCaught(true);
    ...
    return (
        <div>
            ...
            <button type='button' onClick={onClickSearch}>Search for Pokemon!</button>
            {Object.keys(pokemon).length !== 0 &&
                <div>
                    ...
                    <button onClick={catchPokemon} disabled={alreadyCaught}>Try to Catch {pokemon.name}</button>
                </div>
            }
        </div>
    )
